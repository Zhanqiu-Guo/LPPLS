...
class LPPLS(object):
    ...
    def fit(self, max_searches, minimizer='BFGS', obs=None):
        """
        Args:
        ...
        while search_count < max_searches:
            ...
            # originaly uniform
            non_lin_vals = [np.random.uniform(a[0], a[1]) for a in init_limits]
    ...
    def compute_indicators(self, res, filter_conditions_config=None):
        ...
        if filter_conditions_config is None:
            ...
            m_min, m_max = filter_conditions_config[0]
            w_min, w_max =  filter_conditions_config[1]
            O_min =  filter_conditions_config[2]
            D_min =  filter_conditions_config[3]
        ...
        for r in res:
            ...
            pos = pos_count / (pos_count + neg_count) if pos_count > 0 else 0
            neg = neg_count / (pos_count + neg_count) if neg_count > 0 else 0
            pos_lst.append(pos)
            neg_lst.append(neg)
    ...
    def plot_confidence_indicators(self, res):
        ...
        cur_pos_conf = res_df['pos_conf'].iloc[-1]
        cur_neg_conf = res_df['neg_conf'].iloc[-1]
        ...
        ax1_0.plot(ts, res_df['pos'], label='pos bubbles', color='gray', alpha=0.5)
        ...
        ax2_0.plot(ts, res_df['neg'], label='neg bubbles', color='gray', alpha=0.5)
        ...
        # axes up to make room for them
        # fig.autofmt_xdate()
        return cur_pos_conf, cur_neg_conf
    ...
    def compute_nested_fits(self, window_size=80, smallest_window_size=20, outer_increment=5, inner_increment=2,
        ...
        for i in range(0, obs_copy_len + 1, outer_increment):
            ...
            while j < window_delta:
                ...
                for k in [t2, t1, a, b, c, m, 0, tc]:
                    res[i_idx-1][j_idx-1].append(k)
                j += max(window_size // np.sqrt(j), inner_increment)
        return xr.DataArray(
            data=res,
    ...
    def _func_compute_nested_fits(self, args):
        ...
        for j in range(0, window_delta, inner_increment):
            ...
            # j += max(1, int(inner_increment - np.sqrt(j)))
